...
angular.module('arxivar.plugins').factory('comandoCliente', [
    'PluginCommand', 'arxivarResourceService' , 'arxivarUserServiceCreator' , 'arxivarRouteService' , 'arxivarDocumentsService' , 'arxivarNotifierService' , '_',
    ( PluginCommand: IPluginCommand, arxivarResourceService: IArxivarResourceService , arxivarUserServiceCreator: IArxivarUserServiceCreator , arxivarRouteService: IArxivarRouteService , arxivarDocumentsService: IArxivarDocumentsService , arxivarNotifierService: IArxivarNotifierService, _:LoDashStatic) => {
  ...
  // Allowed types are: string, number, boolean or date (Date type is a string UTC ISO 8601 (https://it.wikipedia.org/wiki/ISO_8601) format
	const customSettings: ICustomSettings[]  = [
		{name: 'pluginId', description: 'pluginId', defaultValue:'57040fea-3e82-4521-b299-8329ad365e46', type: 'string'},
	];
  ...
  // Allowed types are: string, number, boolean or date (Date type is a string UTC ISO 8601 (https://it.wikipedia.org/wiki/ISO_8601) format
    ...
    myPlugin.run = (params) => {
		return myPlugin.canRun(params).then((canRun) => {
			if(canRun) {								
				//const doc = params.docnumbers[0];
				const pluginID = 'xyz';
				const url = arxivarRouteService.getURLPluginRoute(pluginID);
				console.log('###url', url);
