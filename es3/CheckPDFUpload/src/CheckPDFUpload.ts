...
angular.module('arxivar.plugins').factory('CheckPDFUpload', [
	'PluginProfilation', 'arxivarResourceService', '$uibModal', '_','arxivarNotifierService',
	function (PluginProfilation: IPluginProfilation, arxivarResourceService: IArxivarResourceService, $uibModal: any, _: LoDashStatic, arxivarNotifierService:IArxivarNotifierService) {
		...
		const requiredSettings: IRequiredSettings = {
			...
			label: 'CheckPDFUpload', // User Interface label (type: string)
			description: 'CheckPDFUpload', // Plugin description (type: string)
		...
		myPlugin.run = function (params) {
			...
			return myPlugin.canRun(params)
				.then(function (canRun) {
					if (canRun) {						
						const returnFiles = params.files;
						return { fields: params.fields, files: returnFiles };
					}
					return null;
				});
		};
